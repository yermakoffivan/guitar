...
std::vector<AbstractCharacterBasedApplication::Char16> *AbstractCharacterBasedApplication::char_screen()
{
	return &m->screen;
}
...
const std::vector<AbstractCharacterBasedApplication::Char16> *AbstractCharacterBasedApplication::char_screen() const
{
	return &m->screen;
}
 ...
 * @return
 */
int AbstractCharacterBasedApplication::internalParseLine(Document::Line *line, int current_col, std::vector<Character> *out_chars, std::vector<CharFlags> *out_flags)
{
	out_chars->clear();
		...
		if (len > 0) {
			...
			while (1) {
				...
				if (col <= current_col && col + n > current_col) {
					index = (int)out_chars->size();
				}
				if (c == 0) break;
				col += n;
				out_chars->emplace_back(c);
			}
		}
	}
	
	if (out_flags) {
		out_flags->resize(out_chars->size());
	}
	
	calc_pos_x(out_chars);
...
void AbstractCharacterBasedApplication::doWrapping()
{
	...
	const int width = m->content_width;
	std::vector<Document::Line> const &original = cx()->engine->document.lines;
	std::vector<Document::Line> wrapped;
	{
		row_index_t linenum = 0;
		for (Document::Line const &line : original) {
			linenum++;
			Document::Line line2 = line;
			std::vector<Character> chrs_in;
			std::vector<std::vector<Character>> chrs_out;
			internalParseLine(&line2, 0, &chrs_in, nullptr);
	...
	cx()->wrapped_lines = std::move(wrapped);
}
...
std::vector<Document::Line> *AbstractCharacterBasedApplication::lines()
{
	if (m->wrapping_mode == WrappingMode::CharWrap) {
		return &cx()->wrapped_lines;
	}
	return &cx()->engine->document.lines;
}

bool AbstractCharacterBasedApplication::isValidRowIndex(row_index_t row_index) const
{
	return row_index >= 0 && row_index < (row_index_t)lines()->size();
}
 ...
 * @return
 */
std::optional<Document::Line> AbstractCharacterBasedApplication::fetchLine(std::vector<Document::Line> const *lines, int row)
{
	// std::vector<Document::Line> const *lines = &engine()->document.lines;
	
	if (row >= 0 && row < lines->size()) {
		Document::Line line = (*lines)[row];
		line.to_vector();
		return line;
	}
	return std::nullopt;
...
void AbstractCharacterBasedApplication::fetchCurrentLine(std::vector<Document::Line> const *lines)
{
...
row_index_t AbstractCharacterBasedApplication::currentVisualRow() const
{
	return cx()->current_visual_row;
}

int AbstractCharacterBasedApplication::currentVisualCol() const
{
	return cx()->current_visual_col;
}

int AbstractCharacterBasedApplication::currentVisualPixelX() const
{
	return cx()->current_visual_pixel_x;
}

void AbstractCharacterBasedApplication::setCurrentVisualRow(row_index_t row)
{
	cx()->current_visual_row = row;
}

void AbstractCharacterBasedApplication::setCurrentVisualCol(int col)
{
...
void AbstractCharacterBasedApplication::invalidateLineFormat(row_index_t row)
{
}
...
void AbstractCharacterBasedApplication::commitLine(std::vector<Character> const &vec)
{
	...
	if (currentLogicalRow() == 0 && lines->empty()) {
	...
	invalidateLineFormat(currentLogicalRow());
}
 ...
 * @param force
 */
void AbstractCharacterBasedApplication::parseCurrentLine(std::vector<Document::Line> const *lines, std::vector<Character> *chars, std::vector<CharFlags> *flags, bool force)
{
	if (force) {
		clearParsedLine();
	}
	...
	if (force || !m->parsed_for_edit) {
		fetchCurrentLine(lines);
		m->current_logical_col = internalParseLine(&m->current_line_data, currentVisualCol(), chars, flags);
 ...
 * @param chars
 */
void AbstractCharacterBasedApplication::parseLine(std::vector<Document::Line> const *lines, int row, std::vector<Character> *chars, std::vector<CharFlags> *flags)
{
	auto opt = fetchLine(lines, row);
	auto *line = opt ? &*opt : nullptr;
	internalParseLine(line, -1, chars, flags);
}
...
bool AbstractCharacterBasedApplication::isCurrentLineWritable() const
{
	...
	if (row >= 0 && row < (int)lines()->size()) {
		if ((*lines())[row].type != Document::LineType::Invalid) {
			return true;
		}
	}
	return false;
...
{//@
	int x = currentVisualCol();
	int w = editorViewportWidth() - RIGHT_MARGIN;
	if (w < 0) w = 0;
	return x > w ? (currentVisualCol() - w) : 0;
}
...
void AbstractCharacterBasedApplication::savePos()
{
	...
	if (p) {
		p->saved_row = currentVisualRow();
		p->saved_col = currentVisualCol();
...
void AbstractCharacterBasedApplication::restorePos()
{
	TextEditorContext *p = editor_cx.get();
	if (p) {
		setCurrentVisualRow(p->saved_row);
		setCurrentVisualCol(p->saved_col);
...
void AbstractCharacterBasedApplication::editSelected(EditOperation op, std::vector<Character> *cutbuffer)
{
	...
	int curr_row = currentVisualRow();
	int curr_col = currentVisualCol();

	setCurrentVisualRow(b.row);
	setCurrentVisualCol(b.col);

	if (a.row == b.row) {
		std::vector<Character> chars;
		parseCurrentLine(lines(), &chars, nullptr, true);
		...
		setCurrentVisualRow(a.row);
		setCurrentVisualCol(a.col);
		int index = calcColumnToIndex(a.col);
		std::vector<Character> chars2;
		parseCurrentLine(lines(), &chars2, nullptr, true);
		if (cutbuffer) {
			std::vector<Character> cut;
...
void AbstractCharacterBasedApplication::doDelete()
{
	...
	parseCurrentLine(lines(), nullptr, nullptr, false);
	...
	if (c == '\n' || c == '\r' || c == -1) {
		...
		if (isSingleLineMode()) {
			...
			if (vec->empty()) {
				clearParsedLine();
				if (currentVisualRow() + 1 < (int)doc->lines.size()) {
					doc->lines.erase(doc->lines.begin() + currentVisualRow());
					moveCursorHome();
				}
			} else {
				commitLine(*vec);
				...
				if (index == (int)vec->size()) {
					...
					if (nextrow < lines) {
						Document::Line *ba1 = &doc->lines[currentVisualRow()];
...
void AbstractCharacterBasedApplication::doBackspace()
{
	...
	if (currentVisualRow() > 0 || currentVisualCol() > 0) {
...
void AbstractCharacterBasedApplication::moveCursorUp()
{
	if (isSingleLineMode()) {
		...
		setCursorRow(currentVisualRow() - 1); // カーソルを1行上へ
		clearParsedLine();
...
void AbstractCharacterBasedApplication::moveCursorDown()
{
	if (isSingleLineMode()) {
		// nop
	} else if (currentVisualRow() + 1 < (int)document()->lines.size()) {
		setCursorRow(currentVisualRow() + 1); // カーソルを1行下へ
		clearParsedLine();
		updateVisibility(true, false, true);
	}
}
...
void AbstractCharacterBasedApplication::moveCursorLeft()
{
	...
	setCursorCol(currentVisualCol() - 1);
	updateVisibility(true, true, true);
}
...
{
	int row = lines()->size();
	if (currentVisualRow() >= row) {
		setCursorPos(row, 0);
		cx()->engine->document.lines.push_back(Document::Line(QByteArray()));
	}
}

void AbstractCharacterBasedApplication::appendNewLine(std::vector<Character> *vec)
{
	if (isSingleLineMode()) return;
...
void AbstractCharacterBasedApplication::writeNewLine()
{
	...
	std::vector<Character> curr_line_chars;
	parseCurrentLine(lines(), &curr_line_chars, nullptr, false);
	int index = m->current_logical_col;
	if (index < 0) {
		addNewLineToBottom();
		index = 0;
	...
	// next line index
	setCurrentVisualRow(currentLogicalRow() + 1);
	// commit current line
	commitLine(curr_line_chars);
	// insert next line
	setCurrentLogicalRow(currentVisualRow()); //@ TODO:
...
{
	out->clear();
	std::vector<Character> const line = m->parsed_current_line_chars;
	int x = 0;
	while (1) {
...
{
	QString text = "[%1:%2]";
	text = text.arg(currentVisualRow() + 1).arg(currentVisualCol() + 1);
	return text;
}
...
{
	SelectionAnchor a;
	a.row = currentVisualRow();
	a.col = currentVisualCol();
	a.enabled = enabled;
	return a;
}
...
void AbstractCharacterBasedApplication::logicalMoveToBottom()
{
	...
	setCurrentVisualRow(documentLines());
	setCurrentVisualCol(0);
	if (currentVisualRow() > 0) {
...
void AbstractCharacterBasedApplication::internalWrite(const ushort *begin, const ushort *end)
{
	...
	parseCurrentLine(lines(), nullptr, nullptr, false);
	int col_index = m->current_logical_col;
	if (col_index < 0) {
		addNewLineToBottom();
		col_index = 0;
	}
	...
	auto WriteChar = [&](uint32_t c){
		if (isInsertMode()) {
			vec->insert(vec->begin() + col_index, Character(c));
